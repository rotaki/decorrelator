...
impl Expr {
    ...
    fn eq(self, other: Expr) -> Expr {
        Expr::Eq {left: Rc::new(self), right: Rc::new(other)}
    }
...
impl RelExpr {
    ...
    fn select(self, mut predicates: Vec<Expr>) -> RelExpr {
        ...
        RelExpr::Select {src: Rc::new(self), predicates}
    }

    fn join(self, other: RelExpr, mut predicates: Vec<Expr>) -> RelExpr {
        // RelExpr::Join {left: Rc::new(self), right: Rc::new(other), predicates}
        for i in 0..predicates.len() {
            ...
            if predicates[i].bound_by(&other) {
                ...
                let predicate = predicates.swap_remove(i);
                return self.join(other, predicates).select(vec![predicate])
            }
        }
    ...
    fn project(self, cols: HashSet<usize>) -> RelExpr {
        RelExpr::Project {src: Rc::new(self), cols}
    }
}
...
impl RelExpr {
    ...
    fn free(&self) -> HashSet<usize> {
        match self {
            ...
            RelExpr::Join {left, right, predicates} => {
                let mut set = left.free();
                set.extend(right.free());
                for pred in predicates {
                    set.extend(pred.free());
                }
                set.difference(&left.att().union(&right.att()).cloned().collect()).cloned().collect()
            },
    ...
    fn att(&self) -> HashSet<usize> {
        match self {
            RelExpr::Scan {table_name: _, column_names} => {
